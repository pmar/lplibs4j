...
public class SparseVector implements Matrix {
     ...
     * @param bottom  Boolean indicating wether the elements are added on the top or bottom
     */
     ...
     *
     * @param v The vector to be concatenated
     */
     ...
     * @return True, if the second vector provides the same values, false otherwise. Note that the
    ...
    // ToDo: structure in the form of unit test!
            ...
            for (int row = 0; row < rows1; row++) {
                for (int column = 0; column < cols2; column++) {
                    if (nonsparseresult[row][column] != res.get(row, column)) {
                        System.out.println("Error in " + row + " " + column + " : " +
                                nonsparseresult[row][column] + " " + res.get(row, column));
                        System.exit(0);
                    }
                }
            }
